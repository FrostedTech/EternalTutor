...
class PromptOrchestrator {
     ...
     */
    validateDeckUsesOwnedCards(deck) {
        for (const deckItem of deck) {
            const collectionItem = this.collection.find((item) => item.card_name.toLowerCase() === deckItem.card_name.toLowerCase() &&
...
DECKLIST:`;
     ...
     */
    parseDecklistResponse(response) {
        const deck = [];
        ...
        for (const line of lines) {
            ...
            try {
                ...
                const collectionItem = this.collection.find((item) => item.card_name.toLowerCase() === cardName.toLowerCase() &&
     ...
     */
    async callOpenRouterLLM(prompt, config) {
     ...
     */
    generateSimulatedLLMResponse(_prompt, config) {
        ...
        for (const requiredName of config.requiredCards) {
            const card = shuffled.find(c => c.card_name.toLowerCase() === requiredName.toLowerCase());
            if (card) {
        ...
        for (const card of shuffled) {
            if (currentTotal >= maxTotalCards)
                break;
            // Skip if this is a required card we already added (to avoid duplicates in commander)
            const isRequired = config.requiredCards.some(req => req.toLowerCase() === card.card_name.toLowerCase());
            ...
            const existingIndex = deck.findIndex(c => c.card_name.toLowerCase() === card.card_name.toLowerCase() &&
            ...
            const toAdd = Math.min(canAdd, card.quantity, spaceLeft);
            if (toAdd > 0) {
                if (existingIndex >= 0) {
